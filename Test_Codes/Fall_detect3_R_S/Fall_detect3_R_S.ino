...
// --- Buttons ---
#define RECALIB_BUTTON_PIN  4   // Button for recalibration
#define SOS_BUTTON_PIN      5   // Button for SOS
...
void setup() {
  ...
  pinMode(RECALIB_BUTTON_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  if (digitalRead(RECALIB_BUTTON_PIN) == LOW) {
    Serial.println("Recalibration button pressed!");
    recalibrateSensor();
    delay(500); // debounce
  }
...
void recalibrateSensor() {
  Serial.println("Recalibrating sensor... Please keep device still.");
  ...
  for (int i = 0; i < samples; i++) {
    ...
    sumX += a.acceleration.x / 9.80665;
    sumY += a.acceleration.y / 9.80665;
    sumZ += a.acceleration.z / 9.80665;
  ...
  offsetZ = (sumZ / samples) - 1.0; // subtract 1g for gravity on Z
